  ...
  return (
    <div className="max-w-2xl mx-auto p-4 pb-12">
      ...
      {completed.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 text-center shadow-sm">
            ...
            <p className="text-xs text-gray-500 mt-1">Рекомендованных</p>
          </div>
          <div className="bg-white rounded-2xl border border-gray-200 p-4 text-center shadow-sm">
            <p className="text-3xl font-bold text-gray-900">{rate}%</p>
            <p className="text-xs text-gray-500 mt-1">Доля рекомендованных</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-6 shadow-sm">
          ...
          {[
            ...
            return (
              <button
                ...
                className={`flex flex-col items-start gap-0.5 px-3 py-2 rounded-xl border text-xs font-medium transition ${
                  checked
              ...
              >
                ...
                <span className={`ml-5 text-xs ${checked ? "text-gray-500" : "text-gray-300"}`}>{sub}</span>
              </button>
            );
          })}
        </div>
        {selectedCases.length === 0 && (
          <p className="text-xs text-red-500 mb-3">Выберите хотя бы один кейс</p>
        )}
        ...
        {selectedCriteria.includes("filler_words") && (
          <div className="mb-3 mt-2 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-gray-800">Допустимое количество слов-паразитов</p>
                <p className="text-xs text-gray-500 mt-0.5 leading-snug">
                  Если кандидат произнесёт больше — вердикт сразу «Не рекомендуется»
                </p>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0">
                <button
                  type="button"
                  onClick={() => setFillerThreshold((v) => Math.max(0, v - 1))}
                  disabled={fillerThreshold === 0}
                  className="w-7 h-7 rounded-lg border border-gray-300 bg-white text-gray-700 font-bold text-sm flex items-center justify-center hover:border-gray-400 transition disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  −
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setFillerThreshold((v) => Math.min(5, v + 1))}
                  disabled={fillerThreshold === 5}
                  className="w-7 h-7 rounded-lg border border-gray-300 bg-white text-gray-700 font-bold text-sm flex items-center justify-center hover:border-gray-400 transition disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  +
                </button>
      ...
      <div>
        ...
        <div className="space-y-3">
          {visibleCompleted.map((s) => {
            const c = s.candidate!;
            const { border, badge } = verdictStyle(c.verdict);
            return (
              <div
                key={s.session_id}
                onClick={() => setSelectedCandidateId(c.id)}
                className={`bg-white rounded-2xl border-2 p-4 shadow-sm cursor-pointer hover:shadow-md transition ${border}`}
              >
